...
- Replaced the earlier `codex exec`-per-document runtime with a long-lived `codex app-server` client in the Electron main process.
- Added assistant IPC methods and shared types for listing Codex chats, creating a new chat, loading a specific Codex thread, sending document-aware prompts into that thread, cancelling active runs, and receiving streamed assistant updates.
...
- Swapped the assistant `New Chat` control to `message-square-plus` and added a matching link-style `New Document` square-pen control to the workspace sidebar header.
...
- Kept every Codex run scoped to the active workspace root while still sending the current document title, document body, workspace path, workspace name, and document path as the primary prompt context.
- Removed Mohio-specific document-thread migration logic because Codex now owns session identity and history.
...
Mohio now includes a real embedded Codex assistant in the desktop shell. Users can browse existing Codex chats for the current workspace, run multiple chats in parallel, and keep using document-first prompts without leaving the app, while the product still avoids silent assistant-driven file mutations.
