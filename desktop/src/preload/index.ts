...
const mohioApi = createMohioApi({
  ...
  openWorkspacePath: (workspacePath) => ipcRenderer.invoke(MOHIO_CHANNELS.openWorkspacePath, workspacePath),
  listRecentWorkspaces: () => ipcRenderer.invoke(MOHIO_CHANNELS.listRecentWorkspaces),
  ...
  getGitCapabilityState: () => ipcRenderer.invoke(MOHIO_CHANNELS.getGitCapabilityState),
  getWorkspaceGitStatus: () => ipcRenderer.invoke(MOHIO_CHANNELS.getWorkspaceGitStatus),
  setWorkspaceGitIdentity: (input) => ipcRenderer.invoke(MOHIO_CHANNELS.setWorkspaceGitIdentity, input),
  syncWorkspaceChanges: () => ipcRenderer.invoke(MOHIO_CHANNELS.syncWorkspaceChanges),
  getAutoSyncStatus: () => ipcRenderer.invoke(MOHIO_CHANNELS.getAutoSyncStatus),
  ...
  connectRemoteRepository: (input) => ipcRenderer.invoke(MOHIO_CHANNELS.connectRemoteRepository, input),
  chooseCloneDestination: () => ipcRenderer.invoke(MOHIO_CHANNELS.chooseCloneDestination),
  cloneRemoteRepository: (input) => ipcRenderer.invoke(MOHIO_CHANNELS.cloneRemoteRepository, input),
